...
class Stack:
    ...
    def add_to_top(self, card):
        ...
        #
        """
        # Change the card's "owner" attribute so that it now belongs to the
        # owner of this stack/deck.
        #   (Note: will be "None" if stack has no owner- e.g., main deck,
        #    weaknesses stack, etc.)
        card.owner = self.owner

        # Updates the stack to include the specified card on TOP of the
        # existing stack.
        ...
        # Resets the stack's "bottom" attribute ONLY if the top and bottom card
        # are the same
    ...
    def add_to_bottom(self, card):
        ...
        # Updates the stack to include the specified card on the BOTTOM of the
        # existing stack.
        ...
        # Resets the stack's "top" attribute ONLY if the top and bottom card are
        # the same
        #   (i.e., there is only one card in the stack).
        if self.left == 1:
    ...
    def remove_top_card(self):
        ...
        # If there are any cards left in the stack, we want to reset the top
        # card.
        # If there aren't, we want to leave the stack empty for now, and set top
        # card to None.
        if self.left > 0:
            self.top = self.stack[0]
        else:
        ...
        # Reset the stack's bottom card ONLY if the top and bottom card are the
        # same
    ...
    def shuffle(self):
        ...
        for i in range(self.left):
            # Create a random integer between that index and the last index in
            # stack.
...
class Deck:
    ...
    def __init__(self, player):
       ...
       :param player:
         ...
         Initializing a Deck object requires a Player object for its sole
         parameter.

         *** NOTE ***  Initializing a Deck object will immediately initialize
                       two Stack objects,
                       both of which have the specified player as their
                       self.owner attribute.

         *** NOTE ***  The game's "Main deck,"" which is not "owned" by any
                       player,
                       is NOT a Deck object, since it does not require two
                       stacks (draw/discard).
    ...
    def shuffle_deck(self):
        ...
        This method should be called ONLY
           ...
           - at the beginning of the game (as part of the "deal_starting_decks"
             Game method)
        ...
        # Make the contents of the draw pile equal to the contents of the
        # discard pile.
    ...
    def discard_top_card(self):
        ...
        This method removes the top card of this deck's draw pile and adds it
        to its discard pile.
...
class Hero:
    ...
    def __init__(self, hero_name, type_bonus=None, active=True):
        ...
        Each Hero is initialized with a name, an optional Cardtype that triggers
        a Hero bonus, and an "Active" state, which will only be made false by
        certain cards in the game. Initializing a Hero object will also give it
        a 'triggered' list, which always begins at zero. This list may be used
        to determine when bonuses are (and are not) given.
    ...
    def card_check(self, card):
        ...
        *** NOTE: This method is called by the "resolve" Card method.
        ...
        # If that Card's cardtype attribute (a string) is identical to this
        # hero's bonus type (another string)...
        if card.cardtype == self.bonus:

            # If this Hero is Superman...
            if self.name == "Superman":

                # Check to see whether this card name has triggered his power
                # yet this turn.
                # If it hasn't...
                if card.name not in self.triggered:

                    # Add it to the triggered list, and return the string
                    # "Power."
            ...
            # If this hero is Nightwing...
                ...
                if len(self.triggered) == 0:
                    ...
                    # Add this card name to the triggered list, and return the
                    # string "Power."
                    self.triggered = self.triggered + [card.name]
                    return "Power"
            ...
            # If this hero is Batman...
                ...
                # Return "Power" regardless of how many times his power has been
                # triggered this turn.
        ...
        # If this hero is Green Lantern...
            ...
            # If this cardtype has not been played yet this turn, add it to the
            # triggered list.
            ...
            if (len(self.triggered) >= 3) and ("yes" not in self.triggered):
                ...
                # Add a string to the triggered list that shows that the power
        ...
        # If this hero is Cyborg...
            ...
            if card.cardtype == "SuperPower":
                ...
                # and this cardtype has not been played this turn, add it to the
                # triggered list.
                if card.cardtype not in self.triggered:
                    self.triggered = self.triggered + [card.cardtype]
...
class Player:
    ...
    def draw_cards(self, count):
        ...
        This method draws the number of cards specified by the "count"
        parameter.
    ...
    def draw_hand(self, handsize=5):
        ...
        Draws a player's hand for the following turn. Default hand size is 5
        cards.
    ...
    def add_to_discard(self, card):
        ...
        This method should be called:
           ...
           - at the start of the game, when each player is dealt their starting
             deck.

           - at the end of each player's turn, to add cards_played to discard
             pile.
    ...
    def gain_card(self, card):
        ...
        if (self.hero.name == "Aquaman") and (card.cost <= 5):
            ...
            # Prompt the player to choose whether to use Aquaman's power on this
            # turn.
            choice = input(
                "Would you like to put this card at the bottom of your deck? "
                "(Y / N) "
            )
        ...
        # If the above condition is not true...
            ...
            if (self.hero.name == "Wonder Woman") and (
                    (card.cardtype == "Villain") or (
                    card.cardtype == "SuperVillain")):
                # Add the card name to the triggered list.
                self.hero.triggered = self.hero.triggered + [card.name]
                print("You get to draw", len(self.hero.triggered),
                      "extra card(s) next turn. How wonderful!")
    ...
    def end_turn(self):
        ...
        This method ends the current player's turn, adds their played hand to
        the discard pile (as well as any cards not played this turn), and draws
        a new hand for that player.
        ...
        *** NOTE *** This method does NOT trigger the next player's turn.
        ...
        # IF we're playing wonder woman, draw extra cards based on how many
        # villains gained this turn.
...
class Card:
    ...
    def __init__(self, name, cardtype, power, draw, vp, cost, defense=False,
                 attack=False, other=False, text=""):
        # Card attributes
        self.name = name
    ...
    def read(self):
        ...
        print(self.name, " (", self.cardtype, ") ", self.power, " Power, ",
              self.vp, " VPs", sep='')
        print(self.text)
    ...
    def resolve(self):
        ...
        if self.owner.hero.card_check(self) == "Power":
            ...
            if (self.owner.name == "Flash") and (
                    self.owner.hero.triggered == []):
                print("You get a bonus draw!")
...
class Game:
    ...
    def show_playorder(self):
        ...
        for i in range(self.num_players):
            print(self.play_order.sequence[i].name, " (",
                  self.play_order.sequence[i].hero.name, ")", sep='')
    ...
    def deal_starting_decks(self):
        ...
        This method deals out the starting deck (seven punches and three
        vulnerabilities) to each player.
...
class Queue:
    ...
    def __init__(self, game, players):
        ...
        for i in range(num_players):
            # Select a random integer between 0 and the last indexed location
            # in the list...
            j = randint(0, num_players - 1)

            # And switch the current Player's location with the Player at that
            # random index.
        ...
        # Now, we want to cycle through the list to see if any of our players
        # are playing Flash.
        for i in range(num_players):

            # If so, switch that Player's place in the queue with whoever is
            # first in the queue.
            if self.sequence[i].hero.name == "Flash":
                self.sequence[i], self.sequence[0] = self.sequence[0], \
                                                     self.sequence[i]

        # Now it's time to make it a true queue, by assigning a "next" attribute
        #  to each item in the queue.
        ...
        if num_players == 2:
            ...
            # But if we have more than two, we need to assign each item's "next"
            # attribute to the player at the next index-- until we get to the
            # final item, that is!
            for i in range(num_players - 1):
                self.sequence[i].next = self.sequence[i + 1]

        # Finally, set the last player's "next" attribute to the first player.
        # (This will be true no matter how many players are in the game, to
        # ensure that we cycle back to the first player upon reaching the end
        # of the play queue.)
        self.last.next = self.first

        # *** NOTE: This is essentially a one-directional queue: each player can
        #           "see" who goes after them, but not who went before them. I
        #           think that should work fine with this game, since it rarely,
        #           as far I'm aware, requires looking backwards in the play
        #           queue. If we discover this to be a more pertinent part of
        #           the game than we thought, however, we should add code to
        #           this section (and a prev attribute to the Player class) to
        #           account for it.


class Turn:

    def __init__(self, player, gamedeck, lineup=None, kicks=None):
        """
       :param player: Sets the player
       :param gamedeck: Sets the Game Deck
       :param lineup: Sets the Lineup in a list
       :param kicks: Sets the kicks in a list
        """
    ...
    def restore_lineup(self, cards=5):
        ...
        for i in range(len(self.lineup)):
            print(self.lineup[i].name, " (Cost: ", self.lineup[i].cost, ")",
                  sep='')
    ...
    def play_phase(self):
        ...
        # Otherwise...
            ...
            print(
                "Please type the name of the card you would like to play next.")
            ...
            # If the player chooses to end their turn, skip to the end_turn
            # phase.
            if prompt == 'end':
                self.final_phase()

            # If the player chooses to, skip to the next phase of the turn:
            # purchasing cards.
            elif prompt == "P" or prompt == "p":
                self.buy_phase()

            # If the player chooses to see their hand again, read hand and
            # return to the beginning of this phase.
            ...
            # Otherwise, play the card chosen by the player.
                ...
                # First, try to find the card typed by the player in the
                # player's hand. If it's there...
                if self.player.find_card_in_hand(prompt):
    ...
    def buy_phase(self):
        ...
        if self.player.power == 0:
            ...
            if self.player.hand:
                choice = input(
                    "You have no power to spend, but there are still cards in "
                    "your hand. Would you like to play a card? (Y / N) ")
            ...
            if choice == "Y":
                ...
                if self.kicks:
                    print("There are also ", len(self.kicks),
                          " Kicks left in the Kicks stack.")
                ...
                elif self.find_card_in_lineup(cardname):
                ...
                if len(self.player.hand) > 0:
                    choice = input(
                        "You have cards left in your hand. Would you like to "
                        "play another card? (Y / N) ")
